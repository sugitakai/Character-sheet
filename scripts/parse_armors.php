...
$file = 'database\data\ゴブスレよく使う防具.txt';
...
for ($i = 0; $i + 3 < count($lines); $i += 4) {
	...
	// 1行目：名前・価格
	preg_match('/【\s*(.+?)\s*】\s*[¥\\\\](\d+|非売品)/u', $lines[$i], $m1);
	...
	// 2行目：属性・装甲値・回避値補正
	preg_match(
		'/属性：【\s*(.+?)\s*】装甲値：【\s*(\d+)\s*】回避値補正：【\s*([\-0-9]+)\s*】/u',
		$lines[$i + 1],
		$m2
	);
	...
	// 3行目：移動力修正・隠密性
	preg_match(
		'/移動力修正：【\s*([\-0-9]+)\s*】隠密性：【\s*(.+?)\s*／\s*([\-0-9]+)\s*】/u',
		$lines[$i + 2],
		$m3
	);
